...
WORKDIR /app
COPY COPYING ./
RUN apt update -qq && apt -y dist-upgrade && apt -y install --no-install-recommends --no-install-suggests tzdata && apt -y install --no-install-recommends --no-install-suggests fprobe nfdump python3-textfsm python3-setuptools python3-colorama git libnet-ssleay-perl libcrypt-ssleay-perl tar zip unzip qrencode s3fs ssh sshpass sshfs samba-client swaks nano wget curl rsync expect iputils-ping pip && apt clean && apt -y autoremove && rm -rf /var/lib/{apt,dpkg,cache,log} && rm -rf /var/cache/* && rm -rf /var/log/apt/* && rm -rf /tmp/*
...
RUN curl "https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip" -o "/root/awscliv2.zip" && unzip /root/awscliv2.zip -d /root && /root/aws/install && rm -f /root/awscliv2.zip && rm -rf /root/aws
